...
/*
  v6 Changes from v5:
  1. Default allocation zeroed — player must build from scratch
  2. Forced liquidation when cash is 0 and capital call due (Y6)
  3. Environment explainers tied to asset class outcomes
  4. Two-step Willow reveal (concept first, then products)
  5. Income visible in dashboard during gameplay
  6. Portfolio-dependent branching decisions (tech surge VC exit, etc.)
  7. Richer narrative connecting decisions to outcomes in results
*/
...
/* ── Environments with explainers (change #3) ── */
const ENVS = [
...
/* ── Decision tree (v6: forced liquidation, portfolio-dependent branches) ── */
function buildDecision(st) {
  ...
  /* Y3: First direct investment opportunity */
  if (st.year === 3) {
    const sorted = CLASSES.map(c => ({cls:c, val:st.positions[c.id+"_core"]||0})).filter(x=>x.val>0).sort((a,b)=>b.val-a.val);
    ...
    return {
      ...
      body: `A ${target.name.toLowerCase()} fund is raising capital for a new vintage. This is a direct investment with a ${lockup}-year lockup and higher return potential than your diversified fund position. The minimum commitment is ${fmt(minInvest)}. Unlike your current allocation, this capital will be locked and unavailable for rebalancing.`,
      options: st.cash >= minInvest ? [
        ...
        { id:"pass", label:"Stay in Diversified Funds", disabled:false, apply: s => ({...s, firstSatellite:null, decisions:[...s.decisions, `Stayed in diversified funds, passed on ${target.name} deal (Y3)`]}) },
      ] : [
        { id:"cant", label:"Pass (Insufficient Cash)", disabled:false, apply: s => ({...s, firstSatellite:null, decisions:[...s.decisions, `Insufficient cash for ${target.name} deal (Y3)`]}) },
      ],
  ...
  /* Y5: Secondary exit (if satellite positions) or new direct deal (if none) */
  if (st.year === 5) {
    ...
    /* Change #6: Portfolio-dependent branch — if VC is largest sat and tech surge, offer premium exit */
    ...
    /* Change #6: If RE is largest sat and rate_hike env, offer distressed exit */
  ...
  /* Change #2: Y6 — Forced liquidation if cash is near 0 (capital call) */
  if (st.year === 6 && st.cash < 5000) {
    ...
    if (corePositions.length === 0) return null;
  ...
  /* Y7: Distribution reinvestment */
  if (st.year === 7 && st.totalIncome > 5000) {
    const inc = Math.round(st.totalIncome);
    return {
      title: "Distribution Reinvestment",
      body: `Your positions have distributed ${fmt(inc)} in income. You can reinvest into your diversified fund positions to compound growth, or add the proceeds to your cash reserve.`,
      options: [
        { id:"reinvest", label:"Reinvest", disabled:false, apply: s => {
          const ct = coreTotal(s); const newPos = {...s.positions};
          if (ct > 0) { CLASSES.forEach(c => { const k=c.id+"_core"; if(newPos[k]>0) newPos[k]+=s.totalIncome*(newPos[k]/ct); }); }
          else { newPos["pc_core"] = (newPos["pc_core"]||0) + s.totalIncome; }
          return {...s, positions:newPos, totalIncome:0, incomeReinvested:true, decisions:[...s.decisions, `Reinvested ${fmt(inc)} in distributions (Y7)`]};
        }},
...
/* Change #7: Richer narrative that connects decisions to outcomes */
function buildNarrative(st) {
  const parts = [];
  const finalVal = totalPortfolio(st);

  if (st.subscriptionFunded === true) {
    parts.push("You deployed additional capital into your funds early, giving those positions more time to compound through subsequent market cycles.");
  } else if (st.subscriptionFunded === false) {
    parts.push("You held cash through the subscription window, preserving liquidity for later opportunities at the cost of early compounding.");
  }
  ...
  if (st.firstSatellite) {
    ...
    if (satVal > 0 && satCls) {
      parts.push(`Your ${satCls.name} direct investment is now worth ${fmt(satVal)}, reflecting the concentrated risk and return profile of satellite positions.`);
    } else if (satCls) {
      parts.push(`You took a direct position in ${satCls.name} but exited before term, capturing partial value.`);
    }
  }

  if (st.premiumExit) {
    parts.push("You sold your VC position at a premium during the tech surge, converting momentum into liquidity.");
  }

  if (st.forcedLiquidation) {
    parts.push("Running without a cash reserve forced a liquidation at a discount when a capital call came due. That loss was the direct cost of full deployment.");
  }

  if (st.earlyExitSold === true && !st.premiumExit) {
    parts.push("Selling a position on the secondary market returned capital at a discount, trading future upside for immediate liquidity.");
  } else if (st.earlyExitSold === false) {
    parts.push("Holding through the secondary offer maintained your exposure through the position's primary value-creation period.");
  }

  if (st.incomeReinvested === true) {
    parts.push("Reinvesting distributions compounded your core positions, though it thinned your cash reserve and may have triggered a liquidity event.");
  } else if (st.incomeReinvested === false) {
    parts.push("Taking distributions as cash preserved your liquidity buffer, giving you flexibility for later commitments.");
  }

  if (st.deferralPenalty) {
    parts.push("The deferral you requested reduced your distribution priority in the final years, costing you income in the home stretch.");
  }

  const incomeClasses = CLASSES.filter(c => c.core.income || c.sat.income);
  const incomeNames = incomeClasses.filter(c => (st.positions[c.id+"_core"]||0) > 0 || (st.positions[c.id+"_sat"]||0) > 0).map(c => c.name);
  if (incomeNames.length > 0 && (st.cumulativeIncome||0) > 1000) {
    parts.push(`Over ten years, your income-generating positions in ${incomeNames.join(" and ")} produced ${fmt(st.cumulativeIncome)} in total distributions.`);
  }
...
function advanceUntilDecision(initState) {
  ...
  while (st.year < 10) {
    ...
    st = { ...st, year:st.year+1, positions:newPos, totalIncome:st.totalIncome+income, cumulativeIncome:(st.cumulativeIncome||0)+income, yearlyIncome:income };
...
function calcResults(hist) {
  ...
  return { totalReturn:(ret*100).toFixed(1), finalValue:e, maxDrawdown:(mdd*100).toFixed(1) };
}
  ...
  return(
    <div style={{display:"flex",alignItems:"center",gap:24,justifyContent:"center",marginBottom:12}}>
      <svg width={180} height={180} viewBox="0 0 180 180">
        ...
        {slices.map(s=>(<circle key={s.id} cx={cx} cy={cy} r={r} fill="none" stroke={s.color} strokeWidth={sw} strokeDasharray={`${s.dash} ${s.gap}`} strokeDashoffset={-s.offset} transform={`rotate(-90 ${cx} ${cy})`} style={{transition:"all .4s"}}/>))}
        {unPct>0&&<circle cx={cx} cy={cy} r={r} fill="none" stroke={B.ltTeal} strokeWidth={sw} strokeDasharray={`${unDash} ${circ-unDash}`} strokeDashoffset={-offset} transform={`rotate(-90 ${cx} ${cy})`} style={{transition:"all .4s"}}/>}
      ...
      <div style={{display:"flex",flexDirection:"column",gap:5}}>
        {CLASSES.map(a=>{const pct=((alloc[a.id]/total)*100).toFixed(0);return alloc[a.id]>0?(<div key={a.id} style={{display:"flex",alignItems:"center",gap:8,fontSize:13,color:B.body}}><span style={{width:10,height:10,borderRadius:2,background:a.color,display:"inline-block",flexShrink:0}}/><span style={{fontWeight:600,minWidth:30}}>{a.short}</span><span style={{color:B.muted}}>{pct}%</span></div>):null;})}
        {reserve>0&&<div style={{display:"flex",alignItems:"center",gap:8,fontSize:13,color:B.body}}><span style={{width:10,height:10,borderRadius:2,background:B.ltTeal,display:"inline-block",flexShrink:0}}/><span style={{fontWeight:600,minWidth:30}}>Cash</span><span style={{color:B.muted}}>{((reserve/total)*100).toFixed(0)}%</span></div>}
      </div>
    </div>
  );
};
...
const RiskReturnChart = ({highlighted})=>{
  ...
  return(
    <svg width="100%" viewBox={`0 0 ${w} ${h}`} style={{display:"block"}}>
      ...
      {[0,.5,1].map((p,i)=>{const y=pad.t+ch*(1-p);return(<g key={i}><line x1={pad.l} x2={w-pad.r} y1={y} y2={y} stroke={B.ltTeal} strokeWidth={.5}/><text x={pad.l-6} y={y+4} textAnchor="end" fontSize={10} fill={B.muted}>{Math.round(p*25)}%</text></g>);})}
      ...
      {CLASSES.map(c=>{
        ...
        return(<g key={c.id}>
          <circle cx={cx1} cy={cy1} r={isHl?6:4} fill={c.color} opacity={isHl?0.5:0.15}/>
          <circle cx={cx2} cy={cy2} r={isHl?6:4} fill={c.color} opacity={isHl?1:0.25} stroke={isHl?B.darkTeal:"none"} strokeWidth={1}/>
          {isHl&&<line x1={cx1} y1={cy1} x2={cx2} y2={cy2} stroke={c.color} strokeWidth={1} strokeDasharray="2,2" opacity={0.4}/>}
          <text x={cx2} y={cy2-10} textAnchor="middle" fontSize={10} fontWeight={isHl?600:400} fill={isHl?B.darkTeal:B.muted}>{c.short}</text>
        </g>);
      })}
    </svg>
  );
...
const AssetDetail = ({cls})=>(
  <div style={{background:B.cream,borderRadius:8,padding:"16px 18px",marginTop:8,marginBottom:4}}>
    <div style={{maxWidth:320,marginBottom:14}}><RiskReturnChart highlighted={cls.id}/></div>
    <div style={{fontSize:10,color:B.muted,marginBottom:8}}>Faded dot = diversified fund. Solid dot = direct investment.</div>
    <div style={{fontSize:13,color:B.body,lineHeight:1.6,marginBottom:10}}>{cls.edu}</div>
    <div style={{display:"flex",flexDirection:"column",gap:4,marginBottom:8}}>
      {cls.traits.map((t,i)=>(<div key={i} style={{fontSize:11,color:B.teal,display:"flex",alignItems:"baseline",gap:6}}><span style={{width:4,height:4,borderRadius:1,background:cls.color,flexShrink:0,marginTop:4}}/><span>{t}</span></div>))}
    </div>
  </div>
);
...
/* Change #3: Environment explainer during gameplay */
const EnvExplainer = ({env})=>{
  const [open, setOpen] = useState(false);
  if (!env || !env.explainer) return null;
  return(
    <div style={{marginBottom:16}}>
      <div style={{background:B.cream,borderRadius:8,padding:"12px 16px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <EventTag env={env}/>
          <span style={{fontSize:11,color:B.teal,cursor:"pointer",fontWeight:500}} onClick={()=>setOpen(!open)}>{open?"Hide":"Why it matters"}</span>
        </div>
        {open && <div style={{fontSize:13,color:B.body,lineHeight:1.6,marginTop:10,paddingTop:10,borderTop:`1px solid ${B.ltTeal}`}}>{env.explainer}</div>}
      </div>
    </div>
  );
};

/* Change #5: Income tracker component for gameplay dashboard */
const IncomeTracker = ({gs})=>{
  ...
  const incomeByClass = CLASSES.filter(c => c.core.income || c.sat.income).map(c => {
    ...
    const coreInc = c.core.income ? coreVal * c.core.incomeRate : 0;
    const satInc = c.sat.income ? satVal * c.sat.incomeRate : 0;
    return { cls: c, estimated: coreInc + satInc };
  }).filter(x => x.estimated > 100);

  return(
    <div style={{background:B.cream,borderRadius:8,padding:"12px 16px",marginBottom:12}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
        <div style={{fontSize:10,fontWeight:600,textTransform:"uppercase",letterSpacing:".12em",color:B.muted}}>Income Generated</div>
        <div style={{fontSize:10,color:B.muted}}>Cumulative: {fmt(cumInc)}</div>
      </div>
      <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
        {incomeByClass.map(x=>(
          <div key={x.cls.id} style={{display:"flex",alignItems:"center",gap:6,fontSize:12,color:B.body}}>
            <span style={{width:8,height:8,borderRadius:2,background:x.cls.color,display:"inline-block",flexShrink:0}}/>
            <span style={{fontWeight:500}}>{x.cls.short}</span>
            <span style={{color:B.muted}}>~{fmt(x.estimated)}/yr</span>
          </div>
        ))}
      </div>
...
/* Change #4: Two-step Framework Reveal (concept first, then Willow products) */
const FrameworkReveal = ({positions, cash, gs})=>{
  ...
  return(
    <div style={{background:B.cream,borderRadius:10,padding:"24px 20px",marginBottom:20,borderTop:`3px solid ${B.teal}`}}>
      {/* Step 1: The concept */}
      <div style={{fontSize:10,fontWeight:600,textTransform:"uppercase",letterSpacing:".18em",color:B.teal,marginBottom:6}}>WHAT YOU BUILT</div>
      <div style={{fontSize:18,fontWeight:500,color:B.darkTeal,lineHeight:1.3,marginBottom:8}}>Core / Satellite / Liquidity</div>
      <div style={{fontSize:14,color:B.body,lineHeight:1.6,marginBottom:16}}>
        The portfolio you constructed follows a framework used by institutional investors worldwide. Your diversified fund positions form the <strong>core</strong>, providing broad exposure and periodic liquidity. Direct investments form the <strong>satellite</strong>, adding concentrated return potential with lockups. Unallocated cash is your <strong>liquidity reserve</strong>, the buffer that lets you meet obligations and seize opportunities.
      </div>

      {/* Stacked bar */}
      <div style={{display:"flex",borderRadius:6,overflow:"hidden",height:28,marginBottom:16}}>
        {cPct>0&&<div style={{width:`${cPct}%`,background:B.green,display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{fontSize:11,fontWeight:600,color:B.white}}>{cPct}% Core</span></div>}
        {sPct>0&&<div style={{width:`${sPct}%`,background:"#d4820a",display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{fontSize:11,fontWeight:600,color:B.white}}>{sPct}% Satellite</span></div>}
        {lPct>0&&<div style={{width:`${lPct}%`,background:B.muted,display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{fontSize:11,fontWeight:600,color:B.white}}>{lPct}% Liquidity</span></div>}
      </div>

      {[
        {label:"Core",pct:cPct,color:B.green,desc:"Diversified fund positions across asset classes. Broad exposure, institutional management, periodic liquidity windows.",val:cv},
        {label:"Satellite",pct:sPct,color:"#d4820a",desc:"Direct investments and co-investments. Higher return potential with lockups and concentrated risk.",val:sv},
        {label:"Liquidity",pct:lPct,color:B.muted,desc:cash > 0 ? "Your cash reserve earned nothing during the simulation. In practice, this capital should be generating yield while remaining accessible." : "You ended with no liquidity buffer.",val:cash},
      ].map(b=>(
        <div key={b.label} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 0",borderBottom:`1px solid ${B.ltTeal}`}}>
          <div>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:2}}>
              <span style={{width:8,height:8,borderRadius:2,background:b.color,display:"inline-block"}}/>
              <span style={{fontWeight:600,fontSize:13,color:B.darkTeal}}>{b.label}</span>
            </div>
            <div style={{fontSize:11,color:B.muted,marginLeft:16,maxWidth:340}}>{b.desc}</div>
          </div>
          <div style={{textAlign:"right"}}>
            <div style={{fontSize:14,fontWeight:600,color:B.teal}}>{fmt(b.val)}</div>
            <div style={{fontSize:11,color:B.muted}}>{b.pct}%</div>
          </div>
        </div>
      ))}

      {/* Step 2: Willow product mapping (revealed on click) */}
      {!showProducts ? (
        <div style={{marginTop:18,textAlign:"center"}}>
          <div onClick={()=>setShowProducts(true)} style={{display:"inline-block",padding:"13px 32px",borderRadius:6,border:`1px solid ${B.teal}`,color:B.teal,fontWeight:600,fontSize:14,cursor:"pointer",letterSpacing:".02em"}}>See how this maps to real products</div>
        </div>
      ) : (
        <div style={{marginTop:18,padding:"18px 16px",background:B.white,borderRadius:8,border:`1px solid ${B.ltTeal}`}}>
          ...
          <div style={{fontSize:15,fontWeight:500,color:B.darkTeal,marginBottom:10,lineHeight:1.3}}>This framework powers the Willow Wealth platform.</div>
          {[
            {label:"Core",color:B.green,product:"Evergreen funds from Goldman Sachs, Carlyle, and StepStone. Diversified exposure with daily NAV and quarterly liquidity."},
            {label:"Satellite",color:"#d4820a",product:"Direct investments via the Willow marketplace. Higher return potential with lockups and concentrated risk."},
            {label:"Liquidity",color:B.muted,product:"Short Term Notes earn yield on idle capital while preserving access. Your cash never has to sit at zero."},
          ].map(b=>(
            <div key={b.label} style={{padding:"10px 0",borderBottom:`1px solid ${B.ltTeal}`,display:"flex",alignItems:"center",gap:10}}>
              <span style={{width:8,height:8,borderRadius:2,background:b.color,display:"inline-block",flexShrink:0}}/>
              <div>
                <div style={{fontWeight:600,fontSize:13,color:B.darkTeal}}>{b.label}</div>
                <div style={{fontSize:11,color:B.muted,lineHeight:1.4}}>{b.product}</div>
              </div>
            </div>
          ))}
          <div style={{marginTop:16,textAlign:"center"}}>
            <div style={{display:"inline-block",padding:"14px 36px",borderRadius:6,background:B.darkTeal,color:B.lime,fontWeight:600,fontSize:14,cursor:"pointer",letterSpacing:".02em"}}>Build this portfolio on Willow Wealth</div>
          </div>
        </div>
      )}
    </div>
...
export default function App() {
  ...
  /* Change #1: All sliders start at 0 */
  ...
  function startGame() {
    ...
    const initPos = {};
    CLASSES.forEach(c => { initPos[c.id+"_core"] = alloc[c.id]||0; initPos[c.id+"_sat"] = 0; });
    const cashStart = Math.max(0, reserve);
    const init = {
      year:0, positions:initPos, cash:cashStart, totalIncome:0, cumulativeIncome:0, yearlyIncome:0, envs,
  ...
  if(phase==="intro") return(
    <div style={{fontFamily:font,background:B.white,minHeight:"100vh"}}>
      ...
      <div style={{maxWidth:540,margin:"0 auto",padding:"36px 24px"}}>
        {[
          {n:"01",t:"Allocate",d:"Spread your capital across four private markets asset classes. Any amount you leave unallocated is held as a cash reserve."},
          {n:"02",t:"Decide",d:"During the simulation, opportunities and obligations appear: fund subscriptions, direct investments, secondary market offers, and liquidity crunches. Each choice reshapes your portfolio."},
          {n:"03",t:"Review",d:"After ten simulated years, see how your portfolio performed and what your decisions cost or gained you."},
        ].map(s=>(
  ...
  if(phase==="allocate") return(
    <div style={{fontFamily:font,background:B.white,minHeight:"100vh"}}>
      ...
      <div style={{maxWidth:560,margin:"0 auto",padding:"28px 24px"}}>
        ...
        {/* Change #1: Prompt to build from scratch */}
        {totalAlloc === 0 && (
          <div style={{background:"#edfaf2",borderRadius:8,padding:"12px 16px",fontSize:13,color:B.green,marginBottom:16,lineHeight:1.5}}>Use the sliders below to build your allocation from scratch. How you divide this capital will determine every opportunity and constraint you face over the next decade.</div>
        )}
        ...
        {reserve>0&&reserve<500000&&(
          <div style={{background:B.cream,borderRadius:8,padding:"12px 16px",fontSize:13,color:B.body,marginBottom:12,lineHeight:1.5}}>{fmt(reserve)} held as cash reserve. Cash earns no return, but it is the only capital available when commitments come due or opportunities appear.</div>
        )}
        ...
        {showConfirm&&(
          <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,46,48,.55)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100,padding:20}}>
            <div style={{background:B.cream,borderRadius:12,padding:"32px 28px",maxWidth:440,width:"100%",boxShadow:"0 8px 32px rgba(0,0,0,.2)"}}>
              ...
              <div style={{marginBottom:16}}>
                {CLASSES.map(c=>{const amt=alloc[c.id]; if(amt<=0)return null; return(
                  <div key={c.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:`1px solid ${B.ltTeal}`}}>
                    <div style={{display:"flex",alignItems:"center",gap:8}}><span style={{width:8,height:8,borderRadius:2,background:c.color,display:"inline-block"}}/><span style={{fontSize:13,fontWeight:500,color:B.darkTeal}}>{c.name}</span></div>
                    <div><span style={{fontSize:13,fontWeight:600,color:B.teal}}>{fmt(amt)}</span><span style={{fontSize:11,color:B.muted,marginLeft:6}}>{((amt/500000)*100).toFixed(0)}%</span></div>
                  </div>
                );})}
              </div>
              <div style={{background:B.white,borderRadius:8,padding:"12px 14px",marginBottom:20}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div>
                    <div style={{fontSize:10,fontWeight:600,textTransform:"uppercase",letterSpacing:".1em",color:B.muted}}>Cash Reserve</div>
                    <div style={{fontSize:18,fontWeight:500,color:reserve>0?B.teal:B.red,marginTop:2}}>{fmt(Math.max(0,reserve))}</div>
                  </div>
                  <div style={{fontSize:11,color:B.muted,maxWidth:200,textAlign:"right",lineHeight:1.4}}>{reserve>0?"Available for new investments and obligations during the simulation. Earns no return.":"No cash reserve. You may be forced to liquidate positions at a discount when obligations come due."}</div>
                </div>
              </div>
  ...
  if(phase==="play"&&gs) {
    ...
    const changeTot=((tp-500000)/500000)*100;
      ...
      <div style={{fontFamily:font,background:B.white,minHeight:"100vh"}}>
        <div style={{background:B.darkTeal,padding:"20px 28px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
            ...
            <div style={{textAlign:"right"}}>
              <div style={{fontSize:22,fontWeight:500,color:B.lime}}>{fmt(tp)}</div>
        ...
        <div style={{maxWidth:600,margin:"0 auto",padding:"24px 24px"}}>
          ...
          {summary&&(
            <div style={{display:"flex",gap:10,flexWrap:"wrap",margin:"16px 0"}}>
              ...
              <Stat label="Cash Reserve" value={fmt(gs.cash)} sub={gs.cash<30000?"Low":"Available"} color={gs.cash<30000?B.warn:B.teal}/>
            </div>
          )}

          {/* Change #5: Income tracker visible during gameplay */}
          <IncomeTracker gs={gs}/>

          {/* Change #3: Environment explainer */}
          {summary&&summary.env&&<EnvExplainer env={summary.env}/>}

          {/* Historical event tags */}
          {gs.history.filter(h=>h.env).length > 1 && (
            <div style={{marginBottom:16}}>
              <div style={{fontSize:10,fontWeight:600,textTransform:"uppercase",letterSpacing:".12em",color:B.muted,marginBottom:6}}>Market History</div>
            ...
            <div style={{background:B.cream,borderRadius:10,padding:"24px 24px",marginBottom:20}}>
              ...
              <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
                {decision.options.map(opt=>(
                  <Btn key={opt.id} primary={!["decline","pass","hold","cash","cant","force_sell"].includes(opt.id)} disabled={opt.disabled} onClick={()=>handleChoice(opt)} style={{flex:"1 1 auto",minWidth:140}}>
                    {opt.label}
                  </Btn>
                ))}
              </div>
          ...
          <div style={{marginTop:8}}>
            ...
            <AllocBar positions={gs.positions} cash={gs.cash}/>
            <div style={{display:"flex",gap:10,flexWrap:"wrap",marginTop:8}}>
              {CLASSES.map(c=>{const cv=gs.positions[c.id+"_core"]||0;const sv=gs.positions[c.id+"_sat"]||0;if(cv+sv<=0)return null;return(<div key={c.id} style={{fontSize:12,color:B.body}}><span style={{fontWeight:600,color:c.color}}>{c.short}</span> {fmt(cv+sv)}{sv>0&&<span style={{fontSize:10,color:B.muted,marginLeft:3}}>({fmt(sv)} direct)</span>}</div>);})}
      ...
      <div style={{fontFamily:font,background:B.white,minHeight:"100vh"}}>
        ...
        <div style={{maxWidth:640,margin:"0 auto",padding:"28px 24px"}}>
          <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:20}}>
            ...
            <Stat label="Max Drawdown" value={`${results.maxDrawdown}%`} sub="Largest peak-to-trough decline" color={parseFloat(results.maxDrawdown)>20?B.warn:B.teal}/>
          ...
          {/* Change #7: Rich narrative */}
          {narrative&&(
            <div style={{margin:"20px 0",background:B.cream,borderRadius:8,padding:"18px 20px"}}>
              <div style={{fontSize:10,fontWeight:600,textTransform:"uppercase",letterSpacing:".12em",color:B.teal,marginBottom:8}}>YOUR STORY</div>
              <div style={{fontSize:14,color:B.body,lineHeight:1.7}}>{narrative}</div>
            </div>
          )}
          ...
          {/* Change #4: Two-step framework reveal */}
          <FrameworkReveal positions={gs.positions} cash={gs.cash} gs={gs}/>
