...
/*
  v5 Architecture:
  - Brand-free gameplay. No Willow, no product names, no manager names.
  - Player allocates across 4 asset classes. Unallocated = cash reserve (earns 0).
  - All capital enters via "diversified funds" (core). No evergreen/marketplace labels.
  - Decisions introduce "direct investments" (satellite) during simulation.
  - Engine tracks _core and _sat sub-positions per asset class.
  - No benchmark. No grade. Results show raw numbers.
  - Willow brand + framework reveal appears only after results.
*/
...
/* ── Decision tree (brand-free, path-dependent) ── */
...
function buildNarrative(st) {
  const parts = [];
  if (st.subscriptionFunded === true) parts.push("added to your funds early");
  else if (st.subscriptionFunded === false) parts.push("held cash through the subscription window");
  if (st.firstSatellite) parts.push("took a direct investment");
  if (st.earlyExitSold === true) parts.push("sold a position for liquidity");
  else if (st.earlyExitSold === false) parts.push("held through the secondary offer");
  if (st.incomeReinvested === true) parts.push("reinvested distributions");
  else if (st.incomeReinvested === false) parts.push("kept distributions as cash");
...
function advanceUntilDecision(initState) {
  ...
  while (st.year < 10) {
    ...
    // Cash earns nothing (this is the point — revealed at end)
    st = { ...st, year:st.year+1, positions:newPos, totalIncome:st.totalIncome+income };
    const tp = totalPortfolio(st);
    st.history = [...st.history, { year:st.year, positions:{...st.positions}, cash:st.cash, totalPortfolio:tp, env }];
...
/* ── Framework Reveal (Willow brand enters here) ── */
...
export default function App() {
  ...
  const [alloc, setAlloc] = useState({pe:150000, re:120000, pc:120000, vc:60000});
  ...
  function startGame() {
    ...
    const init = {
      year:0, positions:initPos, cash:cashStart, totalIncome:0, envs,
      history:[{year:0, positions:{...initPos}, cash:cashStart, totalPortfolio:500000, env:null}],
      decisions:[], penaltyPe:false, deferralPenalty:false,
      subscriptionFunded:null, firstSatellite:null, earlyExitSold:null, incomeReinvested:null,
    };
    runAdvance(init);
  }
  ...
  function resetAll() {
    setPhase("intro");
    setAlloc({pe:150000, re:120000, pc:120000, vc:60000});
  ...
  if(phase==="allocate") return(
    <div style={{fontFamily:font,background:B.white,minHeight:"100vh"}}>
      ...
      <div style={{maxWidth:560,margin:"0 auto",padding:"28px 24px"}}>
        ...
        {reserve===0&&(
          <div style={{background:"#fdf0ed",borderRadius:8,padding:"12px 16px",fontSize:13,color:B.red,marginBottom:12,lineHeight:1.5}}>You have no cash reserve. During the simulation, new investments and obligations require available cash. With everything allocated, you will not be able to respond to them.</div>
        )}
            ...
            <div style={{background:B.cream,borderRadius:12,padding:"32px 28px",maxWidth:440,width:"100%",boxShadow:"0 8px 32px rgba(0,0,0,.2)"}}>
              ...
              <div style={{background:B.white,borderRadius:8,padding:"12px 14px",marginBottom:20}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  ...
                  <div style={{fontSize:11,color:B.muted,maxWidth:200,textAlign:"right",lineHeight:1.4}}>{reserve>0?"Available for new investments and obligations during the simulation. Earns no return.":"No cash reserve. You will not be able to fund new opportunities."}</div>
                </div>
              </div>
            ...
            <div style={{background:B.cream,borderRadius:10,padding:"24px 24px",marginBottom:20}}>
              ...
              <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
                {decision.options.map(opt=>(
                  <Btn key={opt.id} primary={!["decline","pass","hold","cash","cant"].includes(opt.id)} disabled={opt.disabled} onClick={()=>handleChoice(opt)} style={{flex:"1 1 auto",minWidth:140}}>
                    {opt.label}
                  </Btn>
                ))}
              </div>
    ...
    return(
      <div style={{fontFamily:font,background:B.white,minHeight:"100vh"}}>
        <div style={{background:B.darkTeal,padding:"36px 28px",textAlign:"center"}}>
          ...
          {buildNarrative(gs)&&<div style={{fontSize:13,color:B.ltTeal,marginTop:12,maxWidth:440,margin:"12px auto 0",lineHeight:1.5,fontStyle:"italic"}}>{buildNarrative(gs)}</div>}
        </div>
        <div style={{maxWidth:640,margin:"0 auto",padding:"28px 24px"}}>
          <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:20}}>
            ...
            <Stat label="Cash Reserve" value={fmt(gs.cash)} sub="Earned nothing"/>
          </div>
